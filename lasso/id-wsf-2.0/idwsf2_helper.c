		...
		gboolean create)
{
	...
	if (LASSO_IS_IDWSF2_DISCO_SECURITY_CONTEXT (context)) {
		lasso_foreach_full_begin(LassoMiscTextNode*, textnode, it2, context->SecurityMechID);
		if (LASSO_IS_MISC_TEXT_NODE (textnode)) {
			if (g_strcmp0 (textnode->content, security_mech_id) || sech_mech_predicate(textnode->content)) {
				return context;
			}
		}
